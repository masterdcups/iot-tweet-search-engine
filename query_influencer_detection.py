...
class QueryInfluencerDetection:
	...
	def get_influencers(self, topic_vector, percentage_top_user=.1):
		...
		top_candidates = set([x['user_id'] for x in results[:int(len(results) * percentage_top_user)]])
		return sorted(top_candidates, key=lambda k: k['centrality'], reverse=True)
